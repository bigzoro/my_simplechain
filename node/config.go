...
import (
	...
	"github.com/bigzoro/my_simplechain/accounts/scwallet"
	"github.com/bigzoro/my_simplechain/accounts/usbwallet"
...
type Config struct {
	...
	RaftPort int `toml:",omitempty"` // wsw add

	MonitorPort int `toml:",omitempty"` // wsw add
...
func (c *Config) ResolvePath(path string) string {
	...
	if warn, isOld := isOldGethResource[path]; isOld {
		oldpath := ""
		if c.name() == "sipe" {
			oldpath = filepath.Join(c.KeyStoreDir, path) // 优先从keystore目录下找
		}
		...
		if c.name() == "sipe" {
			oldpath = filepath.Join(c.DataDir, path) // 第二从datadir目录下找
		}
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'sipe' subdirectory of datadir.", oldpath)
	...
	return filepath.Join(c.instanceDir(), path) // 最后从datadir/sipe下找
}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	//	nodeKeyFile := conf.ResolvePath(datadirKey)
	//	nodeCertFile := conf.ResolvePath(datadirCert)
	//	rootCertFile := conf.ResolvePath(datadirCaCert)
	//	if extapi, err := external.NewExternalBackend(conf.ExternalSigner, nodeCertFile, nodeKeyFile, []string{rootCertFile}); err == nil {
...
func (c *Config) SavePid() error {
	instanceDir := filepath.Join(c.DataDir, c.name())
	if err := os.MkdirAll(instanceDir, 0700); err != nil {
		log.Error(fmt.Sprintf("Failed to persist app pid: %v", err))
		return err
	}
	appPidFilePath := filepath.Join(instanceDir, applicationPid)
	return savePid(appPidFilePath)
}
