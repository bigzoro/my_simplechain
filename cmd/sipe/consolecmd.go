...
See https://github.com/simplechain-org/go-simplechain/wiki/JavaScript-Console.`,
	...
	attachCommand = cli.Command{
		...
		ArgsUsage: "[endpoint]",
		Flags:     append(consoleFlags, utils.DataDirFlag, utils.KeyStoreDirFlag),
		Category:  "CONSOLE COMMANDS",
		Description: `
The Geth console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
...
func remoteConsole(ctx *cli.Context) error {
	...
	if ctx.GlobalIsSet(utils.KeyStoreDirFlag.Name) {
		...
		keystoreDir, _ = filepath.Abs(filepath.Join(path, utils.KeyStoreDirFlag.Name))
	}
	oldpath := ""
	if path != "" && common.FileExist(filepath.Join(path, "node.crt")) && common.FileExist(filepath.Join(path, "node.key")) {
		oldpath = path
	} else {
		oldpath = keystoreDir
	}
	client, err := dialRPC(endpoint, filepath.Join(oldpath, "node.crt"), filepath.Join(oldpath, "node.key"), []string{filepath.Join(keystoreDir, "ca.crt")})
	if err != nil {
...
func dialRPC(endpoint string, certFile string, keyFile string, certFiles []string) (*rpc.Client, error) {
	if endpoint == "" {
	...
	return rpc.Dial(endpoint, certFile, keyFile, certFiles)
}
