...
Use "ethereum dump 0" to dump the genesis block.`,
	...
	genScretKeyCommand = cli.Command{
		Action:    utils.MigrateFlags(geneBLS12381Sec),
		Name:      "genbls12sec",
		Usage:     "generate a bls12-385 curve based secret key",
		ArgsUsage: " ",
		Flags: []cli.Flag{
			utils.DataDirFlag,
		},
		Category: "BLOCKCHAIN COMMANDS",
		Description: `
		The genbls12sec command generate a bls12-385 curve based secret key and save it to the file "hotstuff-sec".
		`,
	}
	aggregateCommand = cli.Command{
		Action:   utils.MigrateFlags(aggregateBLS),
		Name:     "aggregatebls",
		Usage:    "Aggregate bls partial signature.",
		Flags:    []cli.Flag{},
		Category: "BLOCKCHAIN COMMANDS",
	}
)
...
func initGenesis(ctx *cli.Context) error {
	...
	var snap interface {
		Store(ethdb.Database) error
		Hash() (hash common.Hash)
	}
	if genesis.Config.Hotstuff != nil {
		conf := genesis.Config.Hotstuff
		ids := make([]hots.ID, len(conf.Council))
		pks := make([]*bls.PublicKey, 0, len(conf.Council))
		for i := range conf.Council {
			ids[i].SetUint32(conf.Council[i].ID)
			pubkey := new(bls.PublicKey)
			if err := pubkey.FromBytes(conf.Council[i].PublicKey); err != nil {
				utils.Fatalf("public key recover: %v", err)
			}
			pks = append(pks, pubkey)
		}

		snap = hotstuff.NewSnapshot(ids, pks)
		snaphash := snap.Hash()
		genesis.ExtraData = append(snaphash[:], genesis.ExtraData...)
	}
	...
	for _, name := range []string{"chaindata", "lightchaindata"} {
		...
		if genesis.Config.Hotstuff != nil {
			if err := snap.Store(chaindb); err != nil {
				utils.Fatalf("Failed to write snapshot: %v", err)
			}
		}
		chaindb.Close()
...
func geneBLS12381Sec(ctx *cli.Context) error {
	privkey := bls.GeneratePrivateKey()
	path := node.DefaultDataDir()
	if ctx.GlobalIsSet(utils.DataDirFlag.Name) {
		path = ctx.GlobalString(utils.DataDirFlag.Name)
	}
	path = filepath.Join(path, "hotstuff-sec")

	if err := os.WriteFile(path, privkey.ToBytes(), 0600); err != nil {
		return err
	}

	fmt.Printf("%x", privkey.Public().ToBytes())
	return nil
}

func aggregateBLS(ctx *cli.Context) error {

	input := ctx.Args()
	sigs := make([]*bls.PartialSignature, 0, len(input))
	for i := range input {
		sigbytes, err := hexutil.Decode(input[i])
		if err != nil {
			return err
		}
	...
	aggrbytes, _ := aggr.ToBytes()
	fmt.Printf("0x%x", aggrbytes)
	return nil
}
