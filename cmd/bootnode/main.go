...
func main() {
	var (
		...
		commonName  = flag.String("commonname", "", "commonname")
		keydir      = flag.String("keydir", "", "key dir")
	...
	case *genKey != "":
		...
		if *keydir == "" {
			...
			encodeId := fmt.Sprintf("%x", crypto.FromECDSAPub(&nodeKey.PublicKey)[1:])
			encode := fmt.Sprintf("[\"enode://%s@%s:30303?discport=0&raftport=8548\"]", encodeId, *commonName)
			err = ioutil.WriteFile(filepath.Join(*keydir, "static-nodes.json"), []byte(encode), os.ModePerm)
			if err != nil {
				utils.Fatalf("-nat: %v", err)
			}
			return
