...
import (
	...
	"github.com/bigzoro/my_simplechain/ethstats"
	"github.com/bigzoro/my_simplechain/les"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the SimpleService protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for SimpleService and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // SimpleService address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an SimpleService light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // SimpleService protocol stack
	client *ethclient.Client   // Client connection to the SimpleService chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the SimpleService light client protocol
...
// close terminates the SimpleService connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the SimpleService address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and SimpleService address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the SimpleService address and profile URL.
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No SimpleService address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and SimpleService address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain SimpleService address,
