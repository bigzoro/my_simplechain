...
type Peer struct {
	...
	certificateNumber chan *big.Int
}
...
func newPeer(log log.Logger, conn *conn, protocols []Protocol) *Peer {
	...
	p := &Peer{
		...
		serialNumber:      big.NewInt(0).Set(conn.serialNumber),
		certificateNumber: make(chan *big.Int, 100),
	}
	return p
}
...
loop:
	for {
		...
		case err = <-p.disc:
			reason = discReasonForError(err)
			break loop
		case number := <-p.certificateNumber:
			if p.IsMe(number) {
				break loop
			}
		}
...
// IsMe 数字证书又叫数字凭证、数字标识，它包含了证书持有者的有关信息，以标识他们的身份。
// 数字证书包括的内容有证书持有者的姓名、证书持有者的公钥、公钥的有效期、颁发数字证书的单位、
// 颁发数字证书单位的数字签名和数字证书的序列号。数字证书的序列号保证数字证书的唯一性。
// 检查证书的序列号
func (p *Peer) IsMe(serialNumber *big.Int) bool {
	if p.serialNumber != nil {
		if p.serialNumber.Cmp(serialNumber) == 0 {
			log.Info("Certificate revoked,connection will close")
			return true
		}
	}
	return false
}
func (p *Peer) NotifyCertificate(serialNumber *big.Int) {
	select {
	case p.certificateNumber <- serialNumber:
	default:
		log.Debug("certificateNumber is busy")
	}
}
