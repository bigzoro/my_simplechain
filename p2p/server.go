...
type Server struct {
	...
	secureConnection *secure.SecureConnection

	certificateCh chan types.CertificateEvent
}
...
func (srv *Server) Start() (err error) {
	...
	srv.certificateCh = make(chan types.CertificateEvent, 10)
	srv.running = true
	...
	secureConnection, err := secure.NewSecureConnection(&secure.ConnectionConfig{
		UseTLS: srv.Config.TLSEnable,
		Dir:    srv.Config.TLSDir,
	})
	if err != nil {
		return err
	}
	srv.secureConnection = secureConnection
	// static fields
	...
	if srv.listenFunc == nil {
		srv.listenFunc = srv.secureConnection.Listen
	}
	if srv.Dialer == nil {
		srv.Dialer = srv.secureConnection
	}
	...
	srv.loopWG.Add(1)
	go srv.run(dialer)
	if srv.Config.TLSEnable {
		go srv.runCertificate()
	}
	return nil
}
...
func (srv *Server) setupLocalNode() error {
	...
	srv.ourHandshake = &protoHandshake{Version: baseProtocolVersion, Name: srv.Name, ID: publicKey[1:]}
	...
	if err != nil {
		srv.log.Error("Create the local node", "open db", srv.Config.NodeDatabase, "err", err)
		return err
	}
	srv.nodedb = db
...
running:
	for {
		...
		case op := <-srv.peerOp:
			...
			srv.log.Trace("peerOp", "peerOp", op)
			op(peers)
		...
		case c := <-srv.checkpointAddPeer:
			...
			if err == nil {
				conn, ok := c.fd.(*tls.Conn)
				c.serialNumber = big.NewInt(0)
				if ok {
					state := conn.ConnectionState()
					if len(state.PeerCertificates) > 0 {
						//本节点作为客户端，c作为服务器端state.PeerCertificates[0]肯定有值
						// On the client side, it can't be empty. On the server side, it can be
						// empty if Config.ClientAuth is not RequireAnyClientCert or
						// RequireAndVerifyClientCert.
						c.serialNumber.Set(state.PeerCertificates[0].SerialNumber)
						log.Info("PeerCertificates", "SerialNumber", state.PeerCertificates[0].SerialNumber)
					}
				}
		...
		case pd := <-srv.delpeer:
			...
			if pd.err != nil {
				pd.log.Error("Removing p2p peer", "addr", pd.RemoteAddr(), "peers", len(peers)-1, "duration", d, "req", pd.requested, "err", pd.err)
			}
...
func (srv *Server) setupConn(c *conn, flags connFlag, dialDest *enode.Node) error {
	...
	srv.lock.Unlock()
	if !running {
		srv.log.Trace("running is false", "err", errServerStopped)
	...
	if dialDest != nil {
		...
		if err := dialDest.Load((*enode.Secp256k1)(dialPubkey)); err != nil {
			srv.log.Trace("dial destination doesn't have a secp256k1 public key")
	...
	if dialDest != nil {
		...
		if dialPubkey.X.Cmp(remotePubkey.X) != 0 || dialPubkey.Y.Cmp(remotePubkey.Y) != 0 {
			srv.log.Error("For dialed connections, check that the remote public key matches", "err", DiscUnexpectedIdentity)
...
func nodeFromConn(publicKey *ecdsa.PublicKey, conn net.Conn) *enode.Node {
	var ip net.IP
	var port int
	...
	return enode.NewV4(publicKey, ip, port, port)
}
...
func (srv *Server) GetCertificateCh() chan types.CertificateEvent {
	return srv.certificateCh
}
func (srv *Server) runCertificate() {
	for {
		select {
		case c := <-srv.certificateCh:
			log.Info("receive certificate crl")
			numbers, err := srv.secureConnection.SaveCRL(srv.Config.TLSDir, c.CertificateContent.Content)
			if err != nil {
				fmt.Println("runCertificate", err)
			}
			if numbers != nil {
				for i := range numbers {
					for _, peer := range srv.Peers() {
						if peer != nil {
							peer.NotifyCertificate(numbers[i])
						}
					}
				}
			}
