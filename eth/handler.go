...
import (
	...
	"github.com/bigzoro/my_simplechain/eth/fetcher"
...
type priorBroadcastSelector interface {
	Prior(*types.Block) []enode.ID
}
...
// NewProtocolManager returns a new SimpleService sub protocol manager. The SimpleService sub protocol manages peers capable
// with the SimpleService network.
func NewHandler(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database, cacheLimit int, whitelist map[uint64]common.Hash, pbs priorBroadcastSelector) (*handler, error) {
...
func (h *handler) removePeer(id string) {
	...
	log.Info("handler removePeer", "peer", id)

	// Unregister the peer from the downloader and SimpleService peer set
	err := h.downloader.UnregisterPeer(id)
	if err != nil {
		log.Error("handler UnregisterPeer", "err", err.Error())
	}
...
func (h *handler) Stop() {
	log.Info("Stopping SimpleService protocol")
	...
	log.Info("SimpleService protocol stopped")
}
...
func (h *handler) handle(p *peer) error {
	...
	p.Log().Debug("SimpleService peer connected", "name", p.Name())

	// Execute the SimpleService handshake
	var (
	...
	if err := p.Handshake(h.networkID, td, hash, genesis.Hash(), forkid.NewID(h.blockchain), h.forkFilter); err != nil {
		p.Log().Debug("SimpleService handshake failed", "err", err)
		return err
	}
	if !h.IsValid(p.Node().URLv4()) {
		return errors.New(fmt.Sprintf("check if in permission list,enode %s", p.Node().URLv4()))
	}
	...
	if err := h.peers.Register(p); err != nil {
		p.Log().Error("SimpleService peer registration failed", "err", err)
		return err
	}
	...
	if h.checkpointHash != (common.Hash{}) {
		...
		if err := p.RequestHeadersByNumber(h.checkpointNumber, 1, 0, false); err != nil {
			return err
		}
	...
	for {
		if err := h.handleMsg(p); err != nil {
			p.Log().Debug("SimpleService message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *handler) handleMsg(p *peer) error {
	...
	case msg.Code == GetBlockBodiesMsg:
		...
		log.Debug("send block bodies", "count", len(bodies), "id", p.id)
	...
	case msg.Code == BlockBodiesMsg:
		...
		log.Debug("receive block bodies", "count", len(transactions), "id", p.id)

		// Filter out any explicitly requested bodies, deliver the rest to the downloader
		filter := len(transactions) > 0 || len(uncles) > 0
		if filter {
			transactions, uncles = h.fetcher.FilterBodies(p.id, transactions, uncles, time.Now())
		}
		if len(transactions) > 0 || len(uncles) > 0 || !filter {
			err := h.downloader.DeliverBodies(p.id, transactions, uncles)
			if err != nil {
	...
	case msg.Code == CertificateRevocationListMsg:
		log.Error("receive CertificateRevocationListMsg ")
		object := new(CertificateRevocationListPacket)
		if err := msg.Decode(object); err != nil {
			return fmt.Errorf("message %v: %v", msg, err)
		}
		certificate := &types.CertificateContent{
			Content:   object.Content,
			Signature: object.Signature,
		}
		addr, err := h.ReadAddress(certificate)
		if err != nil {
			return err
		}
		if !h.superManager.IsManager(addr) {
			return errors.New("invalid address")
		}
		//save and broadcast
		h.BroadcastCRL(certificate)
	default:
...
func (h *handler) isSigner(addr common.Address, signers []common.Address) bool {
...
func (h *handler) BroadcastBlock(block *types.Block, propagate bool) {
	...
	if propagate {
		...
		var transfer []*peer
		if h.pbs != nil {
			// Use the selector to send the block to its corresponding priority notification peer.
			eids := h.pbs.Prior(block)
			prior := make(map[enode.ID]struct{}, len(eids))
			for _, eid := range eids {
				prior[eid] = struct{}{}
			}

			transfer = make([]*peer, 0, len(eids))
			for _, peer := range peers {
				if _, ok := prior[peer.Peer.ID()]; ok {
...
func (h *handler) txBroadcastLoop() {
	for {
		select {
		case eventObj := <-h.txsCh:
			h.BroadcastTxs(eventObj.Txs)
...
// NodeInfo represents a short summary of the SimpleService sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // SimpleService network ID (1=Mainnet, 2=Testnet)
...
func (h *handler) BroadcastCRL(certificate *types.CertificateContent) error {
	log.Info("BroadcastCRL...")
	hash := rlpHash(certificate.Content)
	addr, err := h.ReadAddress(certificate)
	if err != nil {
		return err
	}
	...
	h.certificateFeed.Send(types.CertificateEvent{CertificateContent: certificate})
	peers := h.peers.PeersWithoutCRL(hash)
	for _, peer := range peers {
		peer.AsyncSendCRL(certificate)
	}
	return nil
}
func (h *handler) SubscribeCertificateEvent(ch chan<- types.CertificateEvent) event.Subscription {
	return h.scope.Track(h.certificateFeed.Subscribe(ch))
}
...
func (h *handler) ReadAddress(certificate *types.CertificateContent) (common.Address, error) {
	hash := rlpHash(certificate.Content)
	publicKey, err := crypto.SigToPub(hash[:], certificate.Signature)
	if err != nil {
...
func (h *handler) SetCommonDb(commonDb ethdb.Database) {
	h.commonDb = commonDb
}

// IsValid 如果返回true,则表示允许该节点通过验证
func (h *handler) IsValid(nodeId string) bool {
	//todo 简单是否有权限参数生效（true）
	//首先是否已经完成了合约的初始化
	log.Info("handler", "isValid nodeId", nodeId)
	if !h.needCheckPermission {
		log.Info("handler no need to check permission")
		return true
	}
	if h.commonDb != nil {
		has, err := h.commonDb.Has([]byte(common.PermissionInitFinished))
		if err != nil {
			log.Error("handler permissionInitFinished Has", "err", err.Error())
			return false
		}
		if !has {
			log.Info("handler 说明管理合约还没有初始化完成，此时我们允许任意的节点相连")
			//说明管理合约还没有初始化完成，此时我们允许任意的节点相连
			return true
		} else {
			//管理合约已经初始化完成，此时我们需要判断nodeId是否存在，存在则通过，不存在，则不让通过
			has, err := h.commonDb.Has([]byte(h.filterNodeId(nodeId)))
			if err != nil {
				log.Error("handler node Has", "err", err.Error(), "nodeId", nodeId)
				return false
			}
			log.Info("handler has nodeId", "has", has, "nodeId", nodeId)
			//has 为true则通过，false则不通过
			return has
		}
	}
	return false
}
func (h *handler) SetNeedCheckPermission(need bool) {
	h.needCheckPermission = need
}
func (h *handler) filterNodeId(nodeId string) string {
