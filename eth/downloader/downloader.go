		...
		case packet := <-d.headerCh:
			...
			for i, header := range headers {
				...
				if number := header.Number.Int64(); number != expectNumber {
					p.log.Warn("Head headers broke chain ordering", "index", i, "requested", expectNumber, "received", number, "err", errInvalidChain)
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			if skeleton {
				...
				if err != nil {
					p.log.Info("Skeleton chain invalid", "err", err, "return error", errInvalidChain)
			...
			for len(headers) > 0 {
				...
				if d.mode == FastSync || d.mode == LightSync {
					...
					if n, err := d.lightchain.InsertHeaderChain(chunk, frequency); err != nil {
						...
						log.Error("Invalid header encountered", "number", chunk[n].Number, "hash", chunk[n].Hash(), "err", err, "return error", errInvalidChain)
						return errInvalidChain
					}
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		...
		log.Error("importBlockResults", "return error", errInvalidChain, "err", err.Error())
		return errInvalidChain
	}
	return nil
}
...
func (d *Downloader) commitFastSyncData(results []*fetchResult, stateSync *stateSync) error {
	...
	if index, err := d.blockchain.InsertReceiptChain(blocks, receipts, d.ancientLimit); err != nil {
		log.Debug("Downloaded item processing failed", "number", results[index].Header.Number, "hash", results[index].Header.Hash(), "err", err)
		log.Error("commitFastSyncData", "err", err, "return error", errInvalidChain)
		return errInvalidChain
	}
	return nil
}
