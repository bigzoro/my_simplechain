...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux,
	isLocalBlock func(*types.Block) bool, init bool, blockchainCache *core.BlockChainCache) *worker {
	worker := &worker{
		config:             config,
	...
	if _, ok := engine.(consensus.Pbft); ok {
...
func (w *worker) start() {
	...
	if bft, ok := w.engine.(consensus.Pbft); ok {
		err := bft.Start(w.chain, w.blockchainCache, w.eth.TxPool(), w, w.chain.CurrentBlock, w.chain.HasBadBlock)
		if err != nil {
			log.Error("bft.Start", "err", err.Error())
		}
	}
	w.startCh <- struct{}{}
	...
	for {
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				txCount := w.acceptPendingTxs(coinbase, ev.Txs)
				// Only update the snapshot if any new transactons were added
				// to the pending block
				if txCount != w.current.txCount {
					w.updateSnapshot()
				}
			} else {
...
func (w *worker) resultLoop() {
	for {
		...
		case block := <-w.resultCh:
			...
			var (
				_receipts []*types.Receipt
				_state    *state.StateDB
			)
			if _, ok := w.engine.(consensus.Pbft); ok && w.blockchainCache != nil {
				_, _receipts = w.blockchainCache.ReadReceipts(sealHash)
...
func (w *worker) commitNewWork(interrupt *int32, noEmpty bool, timestamp int64) {
	...
	w.applyTransactions(interrupt, noEmpty, start)
}
...
func (w *worker) commit(uncles []*types.Header, interval func(), update bool, start time.Time) error {
	if w.config.NoEmptyBlock && len(w.current.txs) == 0 {
		return nil
	}
	...
	if w.isRunning() {
		...
		case w.taskCh <- &task{receipts: receipts, state: s, block: block, createdAt: time.Now()}:
			...
			log.Info("Commit new mining work", "number", block.Number(), "sealHash", w.engine.SealHash(block.Header()),
				"uncles", len(uncles), "txs", w.current.txCount, "gas", block.GasUsed(), "fees", feesEth, "elapsed", common.PrettyDuration(time.Since(start)))
