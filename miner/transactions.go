...
func (w *worker) applyTransactions(interrupt *int32, noEmpty bool, timeStart time.Time) {
	env := w.current
	...
	if !noEmpty {
		...
		// execution finished.
		w.commit(uncles, nil, false, timeStart)
	}
	...
	w.commit(uncles, w.fullTaskHook, true, timeStart)
}
...
func (w *worker) commitTransactions(txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	for {
		...
		//TxLimit 设置为小于等于0时，不限制交易笔数
		if w.config.TxLimit > 0 && w.current.txCount >= w.config.TxLimit {
			break
		}
		...
		w.current.state.Prepare(tx.Hash(), common.Hash{}, w.current.txCount)
