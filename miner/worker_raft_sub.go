...
// when role change to minter, mined block
func (w *worker) ifPoolHasTx() {
	if w.isRunning() {
		w.requestMinting()
	}
}
...
func (w *worker) commitRaftWork() {
	...
	allTxs := w.eth.TxPool().PendingLimit(header.Number.Uint64(), 1000, true)
	//if err != nil {
	...
	log.Info("Generated next block", "num", block.Number(), "txs", w.current.txCount)
