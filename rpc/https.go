...
const (
	maxRequestContentLength_https = 1024 * 1024 * 5
	contentType_https             = "application/json"
)

// https://www.jsonrpc.org/historical/json-rpc-over-http.html#id13
var acceptedContentTypes_https = []string{contentType_https, "application/json-rpc", "application/jsonrequest"}
...
func DialHTTPSWithClient(endpoint string, client *http.Client) (*Client, error) {
	...
	req.Header.Set("Content-Type", contentType_https)
	req.Header.Set("Accept", contentType_https)
...
func DialHTTPS(endpoint string, certFile string, keyFile string, certFiles []string) (*Client, error) {
	httpClient, err := tls.NewTLSClient(certFile, keyFile, certFiles)
	if err != nil {
		return nil, err
	}
...
func newHTTPSServerConn(r *http.Request, w http.ResponseWriter) ServerCodec {
	body := io.LimitReader(r.Body, maxRequestContentLength_https)
...
func NewHTTPSServer(cors []string, vhosts []string, timeouts HTTPSTimeouts, srv http.Handler) *http.Server {
	...
	if timeouts.ReadTimeout < time.Second {
		log.Warn("Sanitizing invalid HTTP read timeout", "provided", timeouts.ReadTimeout, "updated", DefaultHTTPSTimeouts.ReadTimeout)
		timeouts.ReadTimeout = DefaultHTTPSTimeouts.ReadTimeout
	}
	if timeouts.WriteTimeout < time.Second {
		log.Warn("Sanitizing invalid HTTP write timeout", "provided", timeouts.WriteTimeout, "updated", DefaultHTTPSTimeouts.WriteTimeout)
		timeouts.WriteTimeout = DefaultHTTPSTimeouts.WriteTimeout
	}
	if timeouts.IdleTimeout < time.Second {
		log.Warn("Sanitizing invalid HTTP idle timeout", "provided", timeouts.IdleTimeout, "updated", DefaultHTTPSTimeouts.IdleTimeout)
...
func (s *Server) ServeHTTPS(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("content-type", contentType_https)
...
func validateHttpsRequest(r *http.Request) (int, error) {
	...
	if r.ContentLength > maxRequestContentLength_https {
		err := fmt.Errorf("content length too large (%d>%d)", r.ContentLength, maxRequestContentLength_https)
	...
	if mt, _, err := mime.ParseMediaType(r.Header.Get("content-type")); err == nil {
		for _, accepted := range acceptedContentTypes_https {
			if accepted == mt {
				return 0, nil
			}
		}
	...
	err := fmt.Errorf("invalid content type, only %s is supported", contentType_https)
