...
func (ec *Client) PendingBlock(ctx context.Context) (uint64, error) {
	var result uint64
	err := ec.c.CallContext(ctx, &result, "eth_getLatestBlockNumber")
	return result, err
}
...
func (ec *Client) SendCertificateRevocationList(ctx context.Context, content, signature []byte) error {
	c := &types.CertificateContent{Content: content, Signature: signature}
	data, err := rlp.EncodeToBytes(c)
	if err != nil {
		return err
	}
	return ec.c.CallContext(ctx, nil, "eth_sendCertificateRevocationList", hexutil.Encode(data))
}
