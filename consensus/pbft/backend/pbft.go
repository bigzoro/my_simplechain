...
import (
	...
	pbftCore "github.com/bigzoro/my_simplechain/consensus/pbft/core"
	"github.com/bigzoro/my_simplechain/consensus/pbft/validator"
...
var (
	...
	// errTooFarInFuture
	errTooFarInFuture = errors.New("commit too far in future")
)
var (
...
func (sb *backend) Start(chain consensus.ChainReader, chainWriter consensus.ChainWriter, txPool consensus.TxPool,
	sealer consensus.Sealer, currentBlock func() *types.Block, hasBadBlock func(hash common.Hash) bool) error {
	sb.coreMu.Lock()
...
func ecrecover(header *types.Header) (common.Address, error) {
	...
	bftExtra, err := types.ExtractByzantineExtra(header)
	if err != nil {
		return common.Address{}, err
	}

	// address for seal leader
	addr, err := pbft.GetSignatureAddress(sigHash(header).Bytes(), bftExtra.Seal)
	if err != nil {
		return addr, err
	}
