...
import (
	...
	cmath "github.com/bigzoro/my_simplechain/common/math"
...
func (c *core) GetCurrentBlock() *types.Block {
	if c.current == nil || c.current.Conclusion() == nil {
		return nil
	}
	return c.current.Conclusion().(*types.Block)
}
...
func (c *core) IsHashLocked(blockHash common.Hash) bool {
	return c.current != nil && c.current.IsHashLocked() && c.current.GetLockedHash() == blockHash
}
