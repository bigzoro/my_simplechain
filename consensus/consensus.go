...
type Byzantine interface {
	...
	IsBlockHashLocked(hash common.Hash) bool
}
...
type Pbft interface {
	...
	Start(chain ChainReader, chainWriter ChainWriter, txPool TxPool, sealer Sealer, currentBlock func() *types.Block, hasBadBlock func(hash common.Hash) bool) error

	//SetSealer(sealer Sealer)
	//SetTxPool(pool TxPool)
	//SetChainWriter(writer ChainWriter)
...
type Clique interface {
	Engine
	IsSigner(chain ChainReader, header *types.Header, addr common.Address) bool
	Signers(chain ChainReader, header *types.Header) []common.Address
}
