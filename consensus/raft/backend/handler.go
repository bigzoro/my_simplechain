...
func NewProtocolManager(raftId uint16, raftPort uint16, blockchain *core.BlockChain, mux *event.TypeMux, bootstrapNodes []*enode.Node, joinExisting bool, datadir string, minter *miner.Miner, downloader *downloader.Downloader) (*ProtocolManager, error) {
	...
	raftConfigDir := fmt.Sprintf("%s/sipe", datadir)
...
func (pm *ProtocolManager) resetRaftId(enodeId string, raftId uint16) error {
	node, err := enode.ParseV4(enodeId)
	if err != nil {
		return err
	}
...
func (pm *ProtocolManager) startRaft() {
	...
	//       bugs"
	enablePreVote := true
	defaultLogger := &etcdRaft.DefaultLogger{Logger: etcdlog.New(os.Stderr, "raft", etcdlog.LstdFlags)}
	defaultLogger.EnableDebug()
	logger := etcdRaft.Logger(defaultLogger)
	...
	raftConfig.Logger.Debug("raftlog startRaft", "raft ID", raftConfig.ID)
...
func (pm *ProtocolManager) makeInitialRaftPeers() (raftPeers []etcdRaft.Peer, peerAddresses []*raft.Address, localAddress *raft.Address) {
	...
	for i, node := range initialNodes {
		raftId, err := raft.GetRaftConfigJson(pm.raftConfigPath)
		if err != nil {
			panic(err)
		}
		log.Info("makeInitialRaftPeers", "raft id ", raftId, "pm.raftid", pm.raftId)
