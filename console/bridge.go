...
func (b *bridge) NewAccountByCA(call otto.FunctionCall) (response otto.Value) {
	var (
		commonName string
		password   string
		confirm    string
	...
	case len(call.ArgumentList) == 0:
		...
		if commonName, err = b.prompter.PromptInput("CommonName: "); err != nil {
			throwJSException(err.Error())
		}

	// A single string password was specified, use that
	case len(call.ArgumentList) == 1 && call.Argument(0).IsString():
		commonName, _ = call.Argument(0).ToString()
		if password, err = b.prompter.PromptPassword("Password: "); err != nil {
			throwJSException(err.Error())
		}
		if confirm, err = b.prompter.PromptPassword("Repeat password: "); err != nil {
	...
	// A single string password was specified, use that
	case len(call.ArgumentList) == 2 && call.Argument(0).IsString() && call.Argument(1).IsString():
		password, _ = call.Argument(0).ToString()
		commonName, _ = call.Argument(1).ToString()

	// Otherwise fail with some error
	default:
		throwJSException("expected 0 or 1 or 2 string argument")
	}
	// Password acquired, execute the call and return
	ret, err := call.Otto.Call("jeth.newAccountByCA", nil, password, commonName)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
