...
import (
	"errors"
	"fmt"
	"github.com/simplechain-org/go-simplechain/core/access_contoller"
	"math"
	"math/big"
	"sort"
	"sync"
...
var (
	...
	ErrTransactionFull = errors.New("the transaction pool is full")
)
...
type blockChain interface {
	...
	GetAccessControl() access_contoller.AccessControlProvider
}
...
// 检查一个交易是否符合共识规则以及本地节点的启发式限制（例如，价格和大小）
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	// Heuristic limit, reject transactions over 32KB to prevent DOS attacks
	// 超过64KB的交易将被拒绝，以防止DOS攻击。
	if tx.Size() > 64*1024 {
	...
	// 交易的值不能为负。这在使用RLP解码的交易中通常不会发生，但如果通过RPC创建交易，则可能出现。
	if tx.Value().Sign() < 0 {
		return ErrNegativeValue
	}

	// Ensure the transaction doesn't exceed the current block limit gas.
	// 确保交易的gas量不超过当前块的最大gas限制。
	if pool.currentMaxGas < tx.Gas() {
		log.Error("pool.currentMaxGas", "gas", pool.currentMaxGas)
		log.Error("pool.currentMaxGas", "tx.Gas", tx.Gas())
		return ErrGasLimit
	}
	...
	// 对于非本地交易，如果交易的gas价格低于池子的最小接受gas价格，则将其丢弃。
	local = local || pool.locals.contains(from) // account may be local even if the transaction arrived from the network
	if !local && pool.gasPrice.Cmp(tx.GasPrice()) > 0 {
		log.Error("TxPool validateTx", "pool.gasPrice", pool.gasPrice, "tx.GasPrice()", tx.GasPrice())
	...
	// cost == V + GP * GL
	// 交易发起人应有足够的资金覆盖成本（成本 = 价值 + gas价格 * gas量）。
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (replaced bool, err error) {
	...
	if uint64(pool.all.Count()) >= pool.config.GlobalSlots+pool.config.GlobalQueue {
		return false, ErrTransactionFull
	}
